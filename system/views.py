...
class UpdateRiderDeliveryView(generics.UpdateAPIView):
    """
    API view to update a RiderDelivery.
    - Handles status updates for riders and delivery requests when changed.
    - Accessible only to authenticated users with the appropriate permission.
    """
    queryset = RiderDelivery.objects.all()
    serializer_class = RiderDeliverySerializer
    ...
    def get_object(self):
        ...
        # Ensure the user has permission to update rider deliveries
        if not self.request.user.has_perm('system.change_riderdelivery'):
            raise PermissionDenied({'message': "You do not have permission to update this rider delivery."})

        return rider_delivery

    def update(self, request, *args, **kwargs):
        """
        Handle updating the RiderDelivery instance and managing status changes.
        """
        instance = self.get_object()
        old_rider = instance.rider
        old_delivery_request = instance.delivery_request
        ...
        # Update rider and handle status changes
        new_rider = validated_data.get('rider')
        if new_rider and new_rider != old_rider:
            # Set the old rider's status to "Available"
            old_rider.rider_delivery.current_status = 'Available'
            old_rider.rider_delivery.save()

            # Set the new rider's status to "Assigned"
            new_rider.rider_delivery.current_status = 'Assigned'
            new_rider.rider_delivery.last_assigned_at = timezone.now()
            new_rider.rider_delivery.save()

        # Update delivery request and handle status changes
        new_delivery_request = validated_data.get('delivery_request')
        if new_delivery_request and new_delivery_request != old_delivery_request:
            # Set the old delivery request's status to "Pending"
            old_delivery_request.status = 'Pending'
            old_delivery_request.save()

            # Set the new delivery request's status to "In Progress"
            new_delivery_request.status = 'In Progress'
            new_delivery_request.save()

        # Perform the update
        self.perform_update(serializer)

        return Response({
            "message": "Rider delivery updated successfully.",
            "data": serializer.data
        }, status=status.HTTP_200_OK)